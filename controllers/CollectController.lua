...
function CollectController:Add()
	...
	local db, err = self.mysql

	for k,v in pairs(db) do
		ngx.say(k, type(v))
	end
	
	ret, err, sqlstate, sqlcode = db:query("SELECT NOW()")
	
	ngx.say(self.helper.encode(ret))
	
	for k,v in pairs(ngx.ctx.cleanups) do
...
repeat
	...
	redis:expire(content_key, Config.CONTENT_DETAIL_TTL)
