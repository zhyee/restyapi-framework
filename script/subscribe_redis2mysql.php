 ...
 * @date 2017-10-19
 * @file subscribe_redis2mysql.php
 * php subscribe_redis2mysql.php
 */
...
$logFile = '/data/log/useraction/subscribe_' . date('Y-m-d') . '.log';
 ...
 *     // ---------------------播放记录开始-------------------
    ...
    //while ($hashKeys = $redis->scan($it, $pattern, $count))
		
	$len = $redis->lLen($conf->SUBSCRIBE_AUTHIDS_LIST_KEY);
		
	for($i = 0; $i < $len; $i++)
	{
		$appid_authid = $redis->rPop($conf->SUBSCRIBE_AUTHIDS_LIST_KEY);
		if ($appid_authid)
		{
			list($appid, $authid) = explode('_', $appid_authid);
			$key = $conf->SUBSCRIBE_HASH_KEY_PREFIX . $authid;
			$hash = $redis->hGetAll($key);

            // 先删除该用户的所有订阅
            $sql = "DELETE FROM $SUBSCRIBE_TABLE WHERE authID = '$authid' ";
            mylog($sql);
            $db->query($sql);
            ...
            foreach ($hash as $contentID => $fieldVal)
            {
                if ($index % $step == 0)
                {
                    $insert_query = "INSERT INTO $SUBSCRIBE_TABLE (`authID`,`devID`,`appID`,`productID`,`contentID`,`categoryID`,`contentType`,`creater`,`createTime`,`parentID`,`ext`) VALUES ";
                    $has_data = FALSE;
                }
